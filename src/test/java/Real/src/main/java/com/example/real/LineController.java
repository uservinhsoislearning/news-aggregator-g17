...
//    private HBox TagBox;
    ...
    public void attachValue(LineComponent lineComponent, Stage stage){
        label.setText(STR."\{lineComponent.getCreationDate()}/\{lineComponent.getType()}");
        title.setText(STR." \{lineComponent.getTitle()}");
        author.setText(STR." \{lineComponent.getAuthor()}");
        // bắt đầu tạo button tags
        String[] LTags = lineComponent.getTags().split("\\|");

        previewContent.setOnAction(watchContent ->{
            FXMLLoader loadcontent = new FXMLLoader(getClass().getResource("content-view.fxml"));
            ContentController contentController = new ContentController(stage, firstScene); // Truyền Stage vào LineController
        ...
        title.setOnAction(visitSite -> {
            FXMLLoader loadweb = new FXMLLoader(getClass().getResource("webview.fxml"));
